...
import type { RatioFinancier } from '@/lib/types'
...
function getRatioStatus(
  ...
  // For ratios with min threshold (higher is better)
  ...
  // For ratios with max threshold (lower is better)
...
const statusConfig = {
  good: {
    badge: 'bg-score-favorable-bg text-score-favorable',
    icon: TrendingUp,
    border: 'border-l-score-favorable',
  },
  warning: {
    badge: 'bg-score-reserve-bg text-score-reserve',
    icon: Minus,
    border: 'border-l-score-reserve',
  },
  bad: {
    badge: 'bg-score-defavorable-bg text-score-defavorable',
    icon: TrendingDown,
    border: 'border-l-score-defavorable',
  },
  neutral: {
...
export function RatioCard({ name, ratio }: RatioCardProps) {
  ...
  const getThresholdLabel = () => {
    if (ratio.seuil_min !== undefined)
      return `Seuil min : ${formatValue(ratio.seuil_min, ratio.unite)}`
    if (ratio.seuil_max !== undefined)
      return `Seuil max : ${formatValue(ratio.seuil_max, ratio.unite)}`
    return null
  }
  ...
  return (
    <div
      className={cn(
        'bg-card border border-border rounded-xl p-5 border-l-4',
        config.border
      )}
    >
