...
import { Building2, FolderOpen, Zap, TrendingUp, DollarSign } from 'lucide-react'
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          <StatCard label="Institutions"  value={stats.total_institutions}                          icon={Building2}  color="bg-primary/10 text-primary" />
          <StatCard label="Dossiers"      value={stats.total_dossiers}                              icon={FolderOpen} color="bg-blue-50 text-blue-600" />
          <StatCard label="Tokens LLM"    value={stats.total_tokens_llm.toLocaleString('fr-FR')}    icon={Zap}        color="bg-amber-50 text-amber-600" />
          <StatCard label="Score moyen"   value={`${stats.score_moyen}/100`}                        icon={TrendingUp} color="bg-score-favorable-bg text-score-favorable" />
          <StatCard label="Coût estimé"   value={`$${stats.tokens_cout_estime_usd.toFixed(2)}`}     icon={DollarSign} color="bg-muted text-muted-foreground" sub="Claude API (~$15/M tokens)" />
        </div>
      ) : null}

      <div className="bg-card border border-border rounded-xl p-5">
        <h2 className="text-sm font-semibold text-foreground mb-1">Dossiers créés par jour</h2>
        ...
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={chartData} barSize={14}>
              ...
              <XAxis dataKey="date" tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} tickFormatter={tickFormatter} />
              <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} allowDecimals={false} width={24} />
              <Tooltip
                cursor={{ fill: 'hsl(var(--accent))', radius: 4 }}
                contentStyle={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '8px', fontSize: '12px' }}
                formatter={(v: number) => [v, 'Dossiers']}
              />
              <Bar dataKey="dossiers" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
