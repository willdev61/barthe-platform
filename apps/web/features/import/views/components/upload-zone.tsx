...
export function UploadZone({ onFileSelect, selectedFile, onRemove, disabled = false }: UploadZoneProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center gap-3">
        <div className={cn('flex items-center justify-center w-14 h-14 rounded-full transition-colors', isDragActive ? 'bg-primary/10' : 'bg-muted')}>
          <Upload className={cn('w-6 h-6', isDragActive ? 'text-primary' : 'text-muted-foreground')} />
        </div>
        <div>
          <p className="text-sm font-semibold text-foreground">
            {isDragActive ? 'Déposez le fichier ici' : 'Glissez votre Business Plan'}
          </p>
          <p className="text-xs text-muted-foreground mt-1">PDF, Excel (.xlsx, .xls) ou CSV — taille max 10 Mo</p>
        </div>
        <span className="text-xs text-primary font-medium underline underline-offset-2">Parcourir les fichiers</span>
      </div>
    </div>
  )
}
