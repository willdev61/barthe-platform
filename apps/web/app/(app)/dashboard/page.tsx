...
import { Search, FolderOpen, TrendingUp, Clock, CheckCircle2, Plus, Eye, Trash2, Loader2 } from 'lucide-react'
import { useRouter } from 'next/navigation'
import Link from 'next/link'
import { toast } from 'sonner'
import { getDossiers, deleteDossier } from '@/features/dossiers/services/dossier-service'
...
export default function DashboardPage() {
  ...
  const stats = useMemo(() => {
    if (!dossiers) return { total: 0, enCours: 0, analyses: 0, scoreMoyen: 0 }
    const analyses = dossiers.filter((d) => d.statut === 'analyse')
    const enCours = dossiers.filter((d) => d.statut === 'en_cours' || d.statut === 'en_attente')
    const scores = analyses.map((d) => d.score!).filter((s) => s !== null)
    return {
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard label="Total dossiers" value={stats.total} icon={FolderOpen} color="bg-primary/10 text-primary" />
        <StatCard label="En cours" value={stats.enCours} icon={Clock} color="bg-blue-50 text-blue-600" />
        <StatCard label="Analysés" value={stats.analyses} icon={CheckCircle2} color="bg-score-favorable-bg text-score-favorable" />
        <StatCard label="Score moyen" value={stats.scoreMoyen > 0 ? `${stats.scoreMoyen}/100` : '—'} icon={TrendingUp} color="bg-primary/10 text-primary" />
      </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        {dossiers && filtered.length < dossiers.length && (
