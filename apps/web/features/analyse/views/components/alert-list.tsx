...
const criticiteConfig = {
  ...
  critical: {
    label: 'Critique',
    className: 'bg-score-defavorable-bg border-score-defavorable/30 text-score-defavorable',
...
export function AlertList({ alertes, className }: AlertListProps) {
  if (!alertes || alertes.length === 0) {
    return (
      <div className={cn('flex items-center gap-3 p-4 bg-score-favorable-bg rounded-xl border border-score-favorable/20', className)}>
        <Info className="w-4 h-4 text-score-favorable shrink-0" />
        <p className="text-sm text-score-favorable font-medium">Aucune alerte détectée.</p>
      </div>
    )
  }
    ...
    <div className={cn('space-y-2.5', className)}>
      {sorted.map((alerte) => {
        ...
        return (
          <div
            key={alerte.id}
            className={cn('flex items-start gap-3 p-4 rounded-xl border', config.className)}
            role="alert"
          >
