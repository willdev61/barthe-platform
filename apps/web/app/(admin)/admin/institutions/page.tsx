...
export default function InstitutionsPage() {
  ...
  const handleStatut = async (id: string, current: string) => {
    ...
    try {
      ...
      mutate((prev) =>
        prev?.map((inst) =>
          inst.id === id ? { ...inst, abonnement_statut: next } : inst
        )
      )
    } finally {
      setPendingId(null)
                    ...
                    <td className="px-5 py-3.5 text-center">
                      <span
                        className={cn(
                          ...
                          STATUT_STYLES[inst.abonnement_statut] ?? 'bg-muted text-muted-foreground'
                        )}
                      >
                        ...
                        {deleteConfirm === inst.id ? (
                          <>
                            ...
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-md text-xs font-medium text-muted-foreground hover:bg-accent transition-colors"
                            >
                              Annuler
                            </button>
                          </>
                          ...
                          <>
                            <button
                              ...
                              className={cn(
                                ...
                                inst.abonnement_statut === 'actif'
                                  ? 'text-score-reserve hover:bg-score-reserve/10'
                                  : 'text-score-favorable hover:bg-score-favorable/10',
